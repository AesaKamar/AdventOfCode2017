...
collapsingTests =
  testGroup
    "Testing the collapsing function from parsed repr into tree data structure"
    ...
    , testCase "inserting two unrelated trees" $
      case collapseTrees
             ( [("remwlv", 18, Nothing), ("hello", 18, Nothing)]
             , RootMultiTree []) of
        (unsolved, tree) ->
          assertBool (show unsolved) $
          case tree of
