...
data Command = Increment Integer
             | Decrement Integer deriving (Show, Eq)
...
data Comparison = LessThan
                ...
                | EqualTo
                | NotEqualTo deriving (Show, Eq)

type Predicate = (RegisterName, Comparison, Integer)

data Statement = Statement RegisterName Command Predicate deriving (Show, Eq)
...
statementInterpreter:: Map.Map RegisterName Integer -> Statement ->  Map.Map RegisterName Integer
statementInterpreter m (Statement nameToAlter command predicate) =
  if evaluate m predicate then
    case command of
...
statementFolder strings =
  let parsed = ((parse statementParser "") <$> strings)
      ...
      res =  fmap (\x ->
        Data.Foldable.foldl statementInterpreter Map.empty x) seqParsed
  in res
...
statementInterpreterList:: Map.Map RegisterName [Integer] -> Statement ->  Map.Map RegisterName [Integer]
statementInterpreterList m (Statement nameToAlter command predicate) =
  if evaluateList m predicate then
    case command of
      Increment n ->  Map.insertWith (++) nameToAlter [n] m
      Decrement n ->  Map.insertWith (++) nameToAlter [negate n] m
  else m

statementFolderList :: [String] -> Either ParseError (Map.Map RegisterName [Integer])
statementFolderList strings =
  let parsed = ((parse statementParser "") <$> strings)
      seqParsed = sequence parsed
      res =  fmap (\x ->
        Data.Foldable.foldl statementInterpreterList Map.empty x) seqParsed
  in res
