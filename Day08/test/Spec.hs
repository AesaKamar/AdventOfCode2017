    ...
    describe "Interpreting" $ do
      it "adding to an empty map with a false predicate" $
        statementInterpreter
          Map.empty
          (Statement "b" (Increment 5) ("a", GreaterThan, 1)) `shouldBe`
        Map.empty
      it "adding to an empty map with a true predicate" $
        statementInterpreter
          Map.empty
          (Statement "b" (Increment 5) ("a", EqualTo, 0)) `shouldBe`
        fromList [("b", 5)]
      it "adding to an empty map with a true predicate II" $
        statementInterpreter
          Map.empty
          (Statement "b" (Increment 5) ("a", LessThan, 5)) `shouldBe`
        fromList [("b", 5)]
      it "adding to an nonempty map with a true predicate" $
        statementInterpreter
          (fromList [("a", 5)])
          (Statement "a" (Increment 5) ("a", GreaterThan, 0)) `shouldBe`
        fromList [("a", 10)]
      it
        "adding to an nonempty map with a true predicate and a decrement statement" $
        statementInterpreter
          (fromList [("a", 15)])
          (Statement "a" (Decrement (0)) ("a", GreaterThan, 0)) `shouldBe`
        fromList [("a", 15)]
      it
        "adding to an nonempty map with a true predicate and a negative decrement statement" $
        statementInterpreter
          (fromList [("a", 10)])
          (Statement "a" (Decrement (-15)) ("a", GreaterThan, 0)) `shouldBe`
        fromList [("a", 25)]
      it "yrf dec -813 if jzm != 6" $
        statementInterpreter
          Map.empty
          (Statement "yrf" (Decrement (-813)) ("jzm", NotEqualTo, 6)) `shouldBe`
        fromList [("yrf", (813))]
    describe "Folding" $ do
