...
remainingDistribution size numToDist initialPos pos=
  if relativePos < remainingToDist   then 0
  else 1
  where
...
performDistribution slots =
  let
      ...
      remainderPass = map (remainingDistribution size maxValue maxIndex) [0..size-1]
